...
public:
    ...
    void initialiseReference(const double latitude, const double longitude,
        ...
        geodetic2Ecef(latitude, longitude, altitude, &initial_ecef_x_,
                      &initial_ecef_y_, &initial_ecef_z_);

        // Compute ECEF to NED and NED to ECEF matrices
        double phiP = atan2(initial_ecef_z_, sqrt(pow(initial_ecef_x_, 2) +
    ...
    void ecef2Geodetic(const double x, const double y, const double z,
        ...
        double C = (kFirstEccentricitySquared * kFirstEccentricitySquared * F *
                    r * r) /
                   pow(G, 3);
    ...
    void geodetic2Ned(const double latitude, const double longitude,
                      const double altitude, double *north, double *east,
                      double *down) {
    ...
    void gcj02_to_wgs84(const double latitude, const double longitude,
                        const double altitude, double *east, double *north,
                        double *up) {
        ...
        magic = 1 - kFirstEccentricitySquared * magic * magic;
        double sqrtmagic = std::sqrt(magic);
        dlat = (dlat * 180.0) /
               ((kSemimajorAxis * (1 - kFirstEccentricitySquared)) /
                (magic * sqrtmagic) * M_PI);
        dlng = (dlng * 180.0) /
               (kSemimajorAxis / sqrtmagic * std::cos(radlat) * M_PI);
        double mglat = latitude + dlat;
        double mglng = longitude + dlng;
        geodetic2Enu(latitude * 2 - mglat, longitude * 2 - mglng, altitude,
                     east, north, up);
    }
...
private:
    ...
    double transformlat(const double lng, const double lat) {
        double ret = -100.0 + 2.0 * lng + 3.0 * lat + 0.2 * lat * lat +
                     0.1 * lng * lat + 0.2 * std::sqrt(std::fabs(lng));
        ret += (20.0 * std::sin(6.0 * lng * M_PI) +
                20.0 * std::sin(2.0 * lng * M_PI)) *
               2.0 / 3.0;
        ret +=
            (20.0 * std::sin(lat * M_PI) + 40.0 * std::sin(lat / 3.0 * M_PI)) *
            2.0 / 3.0;
        ret += (160.0 * std::sin(lat / 12.0 * M_PI) +
                320 * std::sin(lat * M_PI / 30.0)) *
               2.0 / 3.0;
        return ret;
    }

    double transformlng(const double lng, const double lat) {
        double ret = 300.0 + lng + 2.0 * lat + 0.1 * lng * lng +
                     0.1 * lng * lat + 0.1 * std::sqrt(std::fabs(lng));
        ret += (20.0 * std::sin(6.0 * lng * M_PI) +
                20.0 * std::sin(2.0 * lng * M_PI)) *
               2.0 / 3.0;
        ret +=
            (20.0 * std::sin(lng * M_PI) + 40.0 * std::sin(lng / 3.0 * M_PI)) *
            2.0 / 3.0;
        ret += (150.0 * std::sin(lng / 12.0 * M_PI) +
                300.0 * std::sin(lng / 30.0 * M_PI)) *
               2.0 / 3.0;
        return ret;
    }
