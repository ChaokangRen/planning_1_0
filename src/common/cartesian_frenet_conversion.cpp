...
void CartesianFrenetConverter::CartesianToFrenet(
...
void CartesianFrenetConverter::FrenetToCartesian(
    const double rs, const double rx, const double ry, const double rtheta,
...
// output s,l only
// void CartesianFrenetConverter::CartesianToFrenet(
//     const double pose_x, const double pose_y, const double pose_rs,
//     const double ref_x, const double ref_y, const double ref_theta,
//     std::array<double, 3> *const frenet_s_ptr,
//     std::array<double, 3> *const frenet_l_ptr) {
//     frenet_s_ptr->at(0) = pose_rs;
//     double determine_the_sign =
//         (pose_y - ref_y) * cos(ref_theta) - (pose_x - ref_x) *
//         sin(ref_theta);
//     frenet_l_ptr->at(0) = copysign(1, determine_the_sign) *
//                           sqrt((pose_x - ref_x) * (pose_x - ref_x) +
//                                (pose_y - ref_y) * (pose_y - ref_y));
// }
void CartesianFrenetConverter::FrentToCartesian(
    const double ref_x, const double ref_y, const double ref_theta,
    const std::array<double, 3> &frenet_s,
    const std::array<double, 3> &frenet_l, double *pose_x_ptr,
    double *pose_y_ptr) {
    *pose_x_ptr = ref_x - frenet_l.at(0) * sin(ref_theta);
    *pose_y_ptr = ref_y + frenet_l.at(0) * cos(ref_theta);
}
void CartesianFrenetConverter::FrentToCartesian(
    const double ref_x, const double ref_y, const double ref_theta,
    const double s, const double l, double *pose_x_ptr, double *pose_y_ptr) {
    std::array<double, 3> frenet_s{s, 0, 0};
    std::array<double, 3> frenet_l{l, 0, 0};
    FrentToCartesian(ref_x, ref_y, ref_theta, frenet_s, frenet_l, pose_x_ptr,
                     pose_y_ptr);
}
}  // namespace planning_lib
}  // namespace jarvis